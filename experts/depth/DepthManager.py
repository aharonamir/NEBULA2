...
class DepthsManager(ExpertManager):
    ...
    @CLI_command
    def local(self, line):
        """run depth detection on local movie file or frames directory, e.g.: local /movies/scenecliptest00581.avi"""
        if os.path.exists(line):
            detector_q = self.pipeline.incoming_queues[DEPTH_DETECTOR]
            try:
    ...
    def remote(self, line):
        """run depth detection on remote movie file, e.g.: remote Movies/92354428"""
        try:
    ...
    class confidence_threshold(global_iou_threshold):
        def __init__(self, default_value=DEPTH_CONFIDENCE_THRESHOLD):
...
class DetectorStep(ExpertPipelineStep):
    ...
    def __save_predictions(self, msg, preds, output_style, output_path):
        ...
        if OUTPUT_STYLE_ANNO in output_style:  # save as annotated video
            DepthAnnotator().annotate_video(video_path, preds, output_path, video_fps=fps, show_pbar=False)
